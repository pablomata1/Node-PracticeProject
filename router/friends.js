...
router.put("/:email", (req, res) => {
    ...
    if(friend){//Check if friend exists
      ...
      //Update DOB, firstName, lastName if provided in request body
      if(DOB){
        friend['DOB'] = DOB;
      } 
      if(firstName){
      ...
      res.send(`Friends with the email ${email} updated.`)
...
router.delete("/:email", (req, res) => {
  //Extract email parameter from reqeust URL
  const email = req.params.email;

  if(email){
    //Delete friends from 'friends' object based on provided email
